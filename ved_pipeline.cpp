...
namespace ved
{
    ...
    void VedPipeline::createGraphicsPipeline(const VedDevice &device, const std::string &vertexFilePath, const std::string &fragFilePath, const PipelineConfigInfo &configInfo)
    {
        ...
        assert(configInfo.renderPass != VK_NULL_HANDLE && "Cannot create graphics pipeline: renderPass not provided in configInfo");
        ...
        if (vkCreateGraphicsPipelines(vedDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeLine) != VK_SUCCESS)
        {
    ...
    {
        std::cout << "Pipeline dimension " << width << "x" << height << std::endl;
